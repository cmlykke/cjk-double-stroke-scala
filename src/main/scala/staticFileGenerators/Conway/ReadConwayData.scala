...
class ReadConwayData {
  ...
  def fileToGraphemeSet(filepath: String): Set[Grapheme] = {
    Grapheme.splitIntoGraphemes(Source.fromFile(filepath).mkString("")).map(x => Grapheme(x)).toSet    
  }
  
  def mapConwayData(): HashMap[Grapheme, ConwayColl] = {
    val bufferedSource = Source.fromFile(conwayFilePath)
    val lines = bufferedSource.getLines
    val missingCedictBufferedSource = Source.fromFile(cedictCharsMissingFromConway)
